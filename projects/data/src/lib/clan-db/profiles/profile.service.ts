...
export class ProfileService {
  ...
  private getProfile(clanId: string, member: ClanMember): Observable<MemberProfile> {
  ...
  getSerializedProfile(clanId: string, member: ClanMember): Observable<MemberProfile> {
    ...
    return this.getProfile(clanId, member).pipe(
      map(
        (profile) =>
          (profileSerializer(profile, this.contentHashService.getProfileHashes()) as unknown) as MemberProfile
      )
    );
  }
}
